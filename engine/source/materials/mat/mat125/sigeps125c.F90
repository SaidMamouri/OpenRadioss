...
!-----------------------------------------------
      ...
      my_real                                                       &
       ...
       e21d,g12,tauxy,epseq,limit_sig
     ...
     !
      ...
      ! membrane computing FS = 0, 1, -1 
       ...
       ! computing damage by direction
        ...
        w11 = one
        w22 = one 
        epseq = sqrt(epsxx(i)**2 + epsyy(i)**2 + half*epsxy(i)**2)
        check(i) = epseq - uvar(i,4)
        uvar(i,4) = epseq
        check(i) = one
        if(epsxx(i)*depsxx(i) < zero .or. epsyy(i)*depsyy(i) < zero .or.        &
           depsxy(i)*epsxy(i) < zero) check(i) = - one
        ...
        ! dir 
         ...
         updat2 = 1
         if(ngl(i) == 11 ) then
            !! write(1,*) tt, w22,check(i)
             if(tt > 100)then
               write(1,*) check(i) , epsxx(i)*depsxx(i),epsyy(i)*depsyy(i)
               write(2,*) depsxy(i)
             endif
         endif
         if(ngl(i) == 11 .and. check(i) < zero) then
            !!write(3,*) 'unloading stress **************'
            !!write(3,*) signxx(i),signyy(i)
            !!write(3,*) 'strain', epsxx(i),epsyy(i)
           !! write(3,*)' ***********************************end'
            !!stop
         endif
         if( check(i) >= zero ) then
           ...
           if(abs(signxx(i)) ==  limit_sig) updat1 = 0
            if(ngl(i)== -11) write(1,*) epsxx(i),signxx(i),updat1
           ...
           if(ngl(i) == 11) then
             write(3,*)epsyy(i),signyy(i), limit_sig 
           endif
           if(abs(signyy(i)) ==  limit_sig) updat2 = 0
            if(ngl(i)== -11) write(2,*) epsyy(i),signyy(i) ,updat2
            if(ngl(i) == 11 .and. updat2 == 0 .and. tt > zero )then
                 print*, 'est ce le cas', tt
                 print*, epsyy(i), em22t, signyy(i)
               !!  stop 

            endif
         endif
         if(updat1 == 0 .and. check(i) >= zero) then
            w11 = signxx(i) / epsxx(i)/e1
            if(ngl(i) == -11) write(1,*) epsxx(i),w11
         endif   
         if(updat2 == 0 .and. check(i) >= zero)  then 
            w22 = signyy(i) / epsyy(i)/e2
             if(ngl(i) == 11) write(2,*)tt,w22,w11
          endif  
         ! save w11 & w22
         if(check(i) >= zero .and. tt <=100) then
          uvar(i,1)= w11
          uvar(i,2)= w22
         endif 
         etse(i)   = one
         ssp(i) = sqrt(max(e1,e2)/rho(i))
         sigy(i)    = sqrt(signxx(i)**2 + signyy(i)**2) ! to check later
                 ...
                 if(check(i) >= zero)then
                  ...
                  if(abs(signxy(i)) >= tau ) then
                    ...
                    if(abs(signxy(i)) > sc) then
                       signxy(i) = sign(slims*sc, signxy(i))
                       updat = 0
                    endif   
                  endif   
                  if(updat == 0 .and. check(i) >= zero)w12 = signxy(i)/epsxy(i)/g12d
                  uvar(i,3)= w12
               enddo ! nel loop
            case(0)
               do i=1,nel
                  signxy(i) = g12*epsxy(i)
                  if(abs(signxy(i)) > sc)signxy(i) = sign(slims*sc, signxy(i))
               enddo ! nel loop
            case(1)
               do i=1,nel
                  signxy(i) = g12*epsxy(i)
                   !!if(ngl(i) == -13) write(1,*)half*epsxy(i),signxy(i)
                  !!if(ngl(i) == 13) write(2,*)half*epsxy(i),signxx(i),signyy(i)
                  if(ngl(i) == 13) write(3,*)tt, signxy(i),signyy(i)
                  if(abs(signxy(i)) > sc)signxy(i) = sign(slims*sc, signxy(i))
               enddo ! nel loop
            end select    
!-------------------------------------------------------------------------------------------
